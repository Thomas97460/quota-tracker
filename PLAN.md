...
## Integration & Multi-Version Testing (Nix-Powered)
To ensure the robustness of the daemon's parsing logic across different provider CLI versions, we will implement a version-stamped integration testing suite.

- **Nix Matrix Shells**: Utilize Nix Flake inputs or overlays to define environments pinned to specific versions of provider CLIs (e.g., `gemini-cli@0.35.0`, `gemini-cli@0.40.1`).
- **Version-Stamped Snapshots**: Maintain a `tests/snapshots` directory containing:
  - Sample configuration and session data generated by specific CLI versions.
  - `expected.json` benchmarks for normalized output.
- **Automated Verification**:
  - `pytest` will iterate through the version matrix, running audit logic against each snapshot.
  - Verify accurate token/quota extraction and correct version detection (stored in `sessions.metadata`).
- **Nix Flake Checks**: Integrate these tests into `nix flake check` to ensure repository-wide compatibility across the full CLI version spectrum.
...
- **Data Collection**:
  - **Quotas (Active Probing)**: Active verification of the real quota at instant T. The daemon must interactively probe provider CLIs (e.g., via `/stats` or specialized status commands) to retrieve live, current usage data.
  - **History (Passive Syncing)**: Periodic polling (every $X$ minutes) to parse local logs and databases for historical token usage.
- **Sync Strategy**:
